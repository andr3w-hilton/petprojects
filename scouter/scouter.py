...
if len(sys.argv) == 1:
	print("Usage: scanner.py <target ip> <interface (default is tun0)>")
	print("Example: scanner.py 10.10.10.10 eth0")
	exit()
elif len(sys.argv) >= 4:
	print("Usage: scanner.py <target ip> <interface (default is tun0)>")
	print("Example: scanner.py 10.10.10.10 eth0")
...
def mass():
	...
	try:
		...
		if a != "":
			...
			print(Style.BRIGHT + Fore.CYAN + '[+] Additional TCP Scan Report' + '\n' + '\n' + Style.RESET_ALL + results + '\n') 
     		else:
			currentDT = datetime.datetime.now()
			zeTime = "[" + currentDT.strftime("%H:%M:%S") + "] "			
			noTCP = 'No additional TCP ports found...'
         		print(Style.BRIGHT + Fore.YELLOW + zeTime + noTCP + Style.RESET_ALL)
	except:
		currentDT = datetime.datetime.now()
		zeTime = "[" + currentDT.strftime("%H:%M:%S") + "] "		
		massWrong = 'Something went wrong! Probably masscan...' 		
...
def nmap1():
	...
	try:
		...
		initialWrong = 'Something went wrong with the initial nmap scan!'
		print(Style.BRIGHT + Fore.YELLOW + zeTime + initialWrong + Style.RESET_ALL + '\n')

def udp():
	global host
	try:	
		b=os.popen("nmap -sU --top-ports 100 " + host + "|grep udp | grep -v filtered | cut -d/ -f1 | tr '\n' ,").read()
		b=b.rstrip("\r\n")
		if b != "":
			currentDT = datetime.datetime.now()
			zeTime = "[" + currentDT.strftime("%H:%M:%S") + "] "			
...
def turbo():
	...
	global interface
	try:
		os.popen("masscan -pU:1-65535 --rate=200 -e " + interface + " " + host + " >/dev/null 2>&1")
	except:
		currentDT = datetime.datetime.now()
		zeTime = "[" + currentDT.strftime("%H:%M:%S") + "] "		
		wrongTurbo = 'Something went wrong with the "turbo function!"'
		print('\n' + Style.BRIGHT + Fore.YELLOW + zeTime + wrongTurbo + Style.RESET_ALL + '\n')
...
if __name__=='__main__':
	...
	print('\n')
	print('\x1B[3m~ Ascertaining Power-Levels Since 1989 ~\x1B[23m')
	print('\n')
	initialScans = 'Running initial scans...'
	print('\n' + Style.BRIGHT + Fore.YELLOW + zeTime + initialScans + Style.RESET_ALL + '\n')
	pmass = Process(target = mass)
	pmass.start()
	pturbo = Process(target = turbo)
	pturbo.start()
